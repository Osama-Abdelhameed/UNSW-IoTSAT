...
The satellite-node hardware platform (Raspberry Pi 4 Model B + Adafruit sensor stack) carries the following sensors and receivers:

| Sensor / receiver | Role |
|---|---|
| Adafruit **INA228** | Power, voltage, and current monitoring |
| Adafruit **LIS3MDL** | 3-axis magnetometer |
| Adafruit **VCNL4040** | Ambient light and proximity |
| Adafruit **PCT2075** | Temperature (spacecraft thermal monitoring) |
| Adafruit **BNO055** | 9-DoF IMU (accelerometer, gyroscope, magnetometer) |
| Adafruit **ICM-20948** | 9-DoF IMU (enhanced motion and orientation) |
| SparkFun **MAX-M10S** (Qwiic) | GNSS receiver |

The source attribution for each released feature is given in the per-column tables further below (see the **Source** column): dataset columns carry their originating sensor, software module, or simulator alongside their `MEASURED` / `COMPUTED` / `SIMULATED` / `LABEL` origin.
